...
#[inline]
pub fn read_u8<R: Read + Unpin + Send>(r: &mut R) -> Result<u8> {
    let mut buf = [0; 1];
...
#[inline]
pub fn read_u16<R: Read + Unpin + Send>(r: &mut R) -> Result<u16> {
    let mut buf = [0; 2];
    r.read_exact(&mut buf)?;
...
#[inline]
pub fn read_u32<R: Read + Unpin + Send>(r: &mut R) -> Result<u32> {
    let mut buf = [0; 4];
    r.read_exact(&mut buf)?;
...
#[inline]
pub fn read_u64<R: Read + Unpin + Send>(r: &mut R) -> Result<u64> {
    let mut buf = [0; 8];
    r.read_exact(&mut buf)?;
...
#[inline]
pub fn read_f32<R: Read + Unpin + Send>(r: &mut R) -> Result<f32> {
    let mut buf = [0; 4];
    r.read_exact(&mut buf)?;
...
#[inline]
pub fn read_f64<R: Read + Unpin + Send>(r: &mut R) -> Result<f64> {
    let mut buf = [0; 8];
    r.read_exact(&mut buf)?;
...
#[inline]
pub fn read_bytes<R: Read + Unpin + Send>(r: &mut R, len: usize) -> Result<Vec<u8>> {
...
#[inline]
pub fn read_str<R: Read + Unpin + Send>(r: &mut R, len: usize) -> Result<String> {
...
#[inline]
pub fn read_key<R: Read + Unpin + Send>(r: &mut R, key: &str) -> Result<()> {
...
#[inline]
pub fn read_list<R: Read + Unpin + Send, T: ReadCbor + Send>(
    r: &mut R,
    len: usize,
) -> Result<Vec<T>> {
...
#[inline]
pub fn read_map<R: Read + Unpin + Send, T: ReadCbor + Send>(
    r: &mut R,
    len: usize,
...
#[inline]
pub fn read_link<R: Read + Unpin + Send>(r: &mut R) -> Result<Cid> {
    let tag = read_u8(r)?;
    if tag != 42 {
...
pub trait ReadCbor: Send + Sized {
    fn try_read_cbor<R: Read + Unpin + Send>(r: &mut R, major: u8) -> Result<Option<Self>>;

    #[inline]
    fn read_cbor<R: Read + Unpin + Send>(r: &mut R) -> Result<Self> {
        let major = read_u8(r)?;
...
impl ReadCbor for bool {
    #[inline]
    fn try_read_cbor<R: Read + Unpin + Send>(_: &mut R, major: u8) -> Result<Option<Self>> {
        match major {
            0xf4 => Ok(Some(false)),
            0xf5 => Ok(Some(true)),
            _ => Ok(None),
