...
impl<'a> serde::Serializer for &'a Serializer {
    ...
    fn serialize_f32(self, value: f32) -> Result<Self::Ok, Self::Error> {
        if value.is_nan() || value.is_infinite() {
            Err(ser::Error::custom(
                "Cannot serialize NaN or infinity for f32",
            ))
        } else {
    ...
    fn serialize_f64(self, value: f64) -> Result<Self::Ok, Self::Error> {
        if value.is_nan() || value.is_infinite() {
            Err(ser::Error::custom(
                "Cannot serialize NaN or infinity for f64",
            ))
        } else {
    ...
    where
        T: ser::Serialize,
    {
        let values = Vec::from([self.serialize_u32(variant_index)?, value.serialize(self)?]);
...
impl ser::SerializeMap for SerializeMap {
        ...
        T: ser::Serialize,
    {
        ...
        self.vec
            .push(Ipld::List(vec![key, value.serialize(&Serializer)?]));
        Ok(())
    }
...
impl<'a> StructSerializer<'a> {
    #[inline]
    fn serialize_field_inner<T>(&mut self, _: &'static str, value: &T) -> Result<(), SerdeError>
    where
    ...
    #[inline]
    fn end_inner(self) -> Result<Vec<Ipld>, SerdeError> {
        Ok(self.vec)
    }
}
