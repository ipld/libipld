...
mod tests {
    ...
    use crate::codec_impl::{IpldCodec, Wrapper};
    use crate::ipld;
    use crate::multihash::{Multihash, SHA2_256};

    type IpldBlock = Block<IpldCodec, Multihash, Wrapper<Ipld>>;

    #[test]
    fn test_references() {
        let b1 = IpldBlock::encode(RAW, SHA2_256, &Wrapper(ipld!(&b"cid1"[..]))).unwrap();
        let b2 = IpldBlock::encode(DAG_JSON, SHA2_256, &Wrapper(ipld!("cid2"))).unwrap();
        let b3 = IpldBlock::encode(DAG_PROTOBUF, SHA2_256, &Wrapper(ipld!("cid3"))).unwrap();

        let payload = Wrapper(ipld!({
            "cid1": &b1.cid,
