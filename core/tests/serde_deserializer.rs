...
/// This function is to test that all IPLD kinds except the given one errors,
/// when trying to deserialize to the given Rust type.
fn error_except<'de, T>(_input: T, except: &Ipld)
where T: Deserialize<'de> + core::fmt::Debug {
  ...
  if !matches!(except, Ipld::Bytes(_)) {
    assert!(
      T::deserialize(Ipld::Bytes(vec![0x68, 0x65, 0x6c, 0x6c, 0x6f])).is_err()
    );
  }
  if !matches!(except, Ipld::List(_)) {
    assert!(
      T::deserialize(Ipld::List(vec![Ipld::Integer(22), Ipld::Bool(false)]))
        .is_err()
    );
  }
  if !matches!(except, Ipld::Map(_)) {
    assert!(
      T::deserialize(Ipld::Map(BTreeMap::from([
...
fn ipld_deserializer_f32_with_loss() {
  // Make sure that there is an error if the value can only be converted with
  // loss. 7.3f32 is different from 7.3f64.
...
fn ipld_deserializer_str() {
  ...
  // TODO vmx 2022-02-09: Doesn't work yet. If we would have a zero-copy
  // version, it should let deserialized = <&str>::deserialize(ipld).unwrap();
...
fn ipld_deserializer_bytes() {
  ...
  // TODO vmx 2022-02-09: Doesn't work yet. If we would have a zero-copy
  // version, it should let deserialized =
  // <&[u8]>::deserialize(ipld).unwrap(); assert_eq!(deserialized, bytes);
}
...
fn ipld_deserializer_tuple_errors() {
  ...
  let ipld_not_matching =
    Ipld::List(vec![Ipld::String(tuple.1.clone()), Ipld::Bool(tuple.0)]);
...
fn ipld_deserializer_option() {
  ...
  // This is similar to `error_except`, which cannot be used here, as we need to
  // excluse `Ipld::Integer` *and* `Ipld::Null`.
  ...
  assert!(
    <Option<u8>>::deserialize(Ipld::List(vec![
      Ipld::Integer(22),
      Ipld::Bool(false)
    ]))
    .is_err()
  ...
  assert!(
    <Option<u8>>::deserialize(Ipld::Map(BTreeMap::from([
      ("hello".into(), Ipld::Null),
      ("world!".into(), Ipld::Float(7.4))
    ])))
    .is_err()
  ...
  assert!(
    <Option<u8>>::deserialize(Ipld::Link(
      Cid::try_from(
        "bafkreie74tgmnxqwojhtumgh5dzfj46gi4mynlfr7dmm7duwzyvnpw7h7m"
      )
      .unwrap()
...
fn ipld_deserializer_enum() {
  ...
  let ipld_one = Ipld::List(vec![Ipld::Integer(0)]);
  ...
  let ipld_two = Ipld::List(vec![Ipld::Integer(1), Ipld::Integer(4)]);
  ...
  let ipld_three = Ipld::List(vec![Ipld::Integer(2), Ipld::Bool(true)]);
...
fn ipld_deserializer_enum_tuple_variant_errors() {
  ...
  let ipld_not_enough = Ipld::List(vec![Ipld::Integer(0), Ipld::Integer(17)]);
  ...
  let ipld_not_matching =
    Ipld::List(vec![Ipld::Integer(0), Ipld::Bool(false), Ipld::Integer(17)]);
...
fn ipld_deserializer_struct_errors() {
  ...
  // let ipld_wrong = Ipld::List(vec![Ipld::Integer(my_struct.hello.into())]);
...
/// This test shows that the values [`serde_json::Value`] supports, can be
...
/// This test shows that CIDs cannot be deserialized into a
/// [`serde_json::Value`].
#[test]
